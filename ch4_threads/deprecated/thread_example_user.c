...
void thread_create(void)
{
        ...
        // set up the thread as user thread 
        pthread_attr_t attr; 
        pthread_attr_setscope(&attr, PTHREAD_SCOPE_PROCESS); 

        if((temp = pthread_create(&thread[0], &attr, thread1, NULL)) == 0)       //comment2
                printf("successfully create thread 1 \n");
        else
                printf("failed to create thread 1\n");
        if((temp = pthread_create(&thread[1], &attr, thread2, NULL)) == 0)  //comment3
