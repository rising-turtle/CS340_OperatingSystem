...
void* count(void * param) // thread function 
{
	...
	printf("I am counting %d dollar bill, and I find %d bills and total is: %d \n", bill_value, sum_cnt, sum_local); 
	sum += sum_local;
	sum_cnt_g += sum_cnt; 
	pthread_exit(0); 
...
int main(int argc, char* argv[]){
	...
	for(int i=0; i<NUM_COUNTERS; i++)
		pthread_create(&counters[i], NULL, count, (char*)(&bill_values[i])); // create thread to run the code 
